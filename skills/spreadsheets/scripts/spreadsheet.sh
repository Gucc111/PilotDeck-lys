...
runtime_ready() {
  ...
  "$node_path" -e '
    ...
    for (const name of ["exceljs", "csv-parse/sync", "iconv-lite", "jszip", "@xmldom/xmldom"]) req.resolve(name);
...
case "${1:-}" in
  ...
  fix)
    ...
    ;;
  ""|-h|--help|help)
    printf 'Usage: spreadsheet.sh <check|fix|inspect|validate|render|deliver|recalculate|compare|convert-legacy|self-test> [options]\n'
    ;;
  *)
    if ! runtime_ready; then
