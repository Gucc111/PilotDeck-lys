...
export default function WorkspaceTeammateBindings({
  ...
  onChange: (teammateId: string, binding: TeammateWorkspaceBinding) => void;
}) {
  ...
  return (
    <div className="space-y-3">
      {teammates.map((teammate) => {
        const binding = bindings[teammate.id] ?? INHERIT_BINDING;
        ...
        const saving = savingId === teammate.id;
        const disabled = savingId !== null;
        const save = (nextBinding: TeammateWorkspaceBinding) =>
        ...
        return (
          <article
            key={teammate.id}
            className="space-y-4 rounded-lg border border-border bg-background/70 p-4"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h4 className="text-sm font-semibold text-foreground">{teammate.name}</h4>
                  <span className={cn(
                ...
                <code className="mt-0.5 block text-[11px] text-muted-foreground">{teammate.id}</code>
              </div>
